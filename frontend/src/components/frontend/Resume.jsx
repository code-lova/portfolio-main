...
const Resume = ({ id }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      if (skillContainer) {
        observer.unobserve(skillContainer);
      }
    };
  }, [hasAnimated]);
  ...
  return (
    ...
    >
      <div className="text-center">
        <h1 className="uppercase text-coral-red tracking-wider text-sm font-bold">
          6+ Years of Experience
        </h1>
        <p className="font-bold text-4xl tracking-wider text-heading-color lg:mt-2">
          My Resume
        </p>
      </div>
      ...
      {/* Education */}
      <div
        className={`mt-20 ${activeSection === "education" ? "" : "hidden"}`}
        id="education"
      >
            ...
            return (
              <div key={edu.id} className="relative flex items-center mb-8">
                ...
                <div className=" ml-6 p-8 card-color shadow-shadow-1 rounded-lg w-full border-1 border-black hover:bg-gradient-to-r from-neutral-900">
                  ...
                  <p className="text-sm font-thin text-heading-color">
                    {edu.school} {edu.year}
                  </p>
      ...
      {/* Professional Skills */}
      <div
        className={`px-8 ${activeSection === "professional-skill" ? "md:grid grid-cols-2 gap-8" : "hidden"} `}
        id="professional-skill"
      >
        <div className="mt-20">
      ...
      <div
        className={`mt-20 ${activeSection === "experience" ? "" : "hidden"}`}
        id="experience"
      >
        <h1 className="px-5 text-coral-red text-center text-3xl">Tech Stack</h1>
        <p className="px-2 tracking-wider text-center font-thin text-[15px] uppercase">
          All tech stacks i have used in web and software development{" "}
        </p>
        <div className="px-5 grid grid-cols-6 gap-2 mt-10 lg:ml-40">
          {technologies.map((tech) => (
            <div
              key={tech.name}
              className="flex w-14 h-[60px] p-2 items-center bg-[#212123] shadow-2xl rounded-md transitioning"
            >
              <img
                src={tech.icon}
                alt="tech stack"
